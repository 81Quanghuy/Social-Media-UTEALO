import axios from 'axios';
import { API_BASE_URL, ACCESS_TOKEN_NAME } from '@/config/serverApiConfig';
import { token as tokenCookies } from '@/auth';
import errorHandler from './errorHandler';
import successHandler from './successHandler';

const headersInstance = { Authorization: 'Bearer ' + tokenCookies.get() };

const axiosInstance = axios.create({
	baseURL: API_BASE_URL,
	timeout: 30000,
	headers: {
		...headersInstance,
	},
});

const request = {
	create: async (entity, jsonData) => {
		try {
			const token = tokenCookies.get();
			const config = {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			};

			let response;

			if (jsonData.postGroupName) {
				response = await axiosInstance.post(
					entity + '/create',
					{
						postGroupName: jsonData.postGroupName,
						bio: jsonData.bio,
						userId: jsonData.userId,
						approvalRequired: false,
						isPublic: jsonData.isPublic,
					},
					config
				);
			} else {
				const formData = new FormData();
				formData.append('location', jsonData.location || '');
				formData.append('content', jsonData.content || '');
				formData.append('postGroupId', jsonData.postGroupId || 0);
				formData.append('privacyLevel', 'ADMIN');

				if (jsonData.files) {
					formData.append('files', jsonData.files);
				}

				if (jsonData.photos) {
					formData.append('photos', jsonData.photos);
				}

				response = await axiosInstance.post(entity + '/create', formData, config);
			}

			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	read: async (entity, id) => {
		axiosInstance.defaults.headers = {
			...headersInstance,
		};
		try {
			const response = await axiosInstance.get(entity + '/read/' + id);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	update: async (entity, id, jsonData) => {
		try {
			const formData = new FormData();
			const token = tokenCookies.get();
			const config = {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			};
			formData.append('userId', id);
			formData.append('isActive', jsonData.isActive);
			formData.append('roleName', jsonData.roleName);

			const response = await axiosInstance.put(entity + '/update', formData, config);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	delete: async (entity, id) => {
		console.log('id', id);
		try {
			const token = tokenCookies.get();
			console.log('token', token);
			const config = {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			};
			const response = await axiosInstance.put(entity + '/delete/' + id, config);
			console.log('response', response);
			console.log('config', config);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	filter: async (entity, option = {}) => {
		axiosInstance.defaults.headers = {
			...headersInstance,
		};
		try {
			let filter = option.filter ? 'filter=' + option.filter : '';
			let equal = option.equal ? '&equal=' + option.equal : '';
			let query = `?${filter}${equal}`;

			const response = await axiosInstance.get(entity + '/filter' + query);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	search: async (entity, source, option = {}) => {
		try {
			let query = '';
			if (Object.keys(option).length !== 0) {
				let fields = option.fields ? 'fields=' + option.fields : '';
				let question = option.question ? '&q=' + option.question : '';
				query = `?${fields}${question}`;
			}
			// headersInstance.cancelToken = source.token;
			const response = await axiosInstance.get(entity + '/search' + query, {
				cancelToken: source.token,
			});

			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	list: async (entity, option = {}) => {
		try {
			const token = tokenCookies.get();
			const config = {
				headers: {
					Authorization: `Bearer ${token}`,
				},
			};

			let query = '';
			if (Object.keys(option).length !== 0) {
				let page = option.page ? 'page=' + option.page : '';
				let items = option.items ? '&items=' + option.items : '';
				query = `?${page}${items}`;
			}
			const response = await axiosInstance.get(entity + '/list' + query, config);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},

	post: async (entityUrl, jsonData, option = {}) => {
		axiosInstance.defaults.headers = {
			...headersInstance,
		};
		try {
			const response = await axiosInstance.post(entityUrl, jsonData);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},
	get: async (entityUrl) => {
		axiosInstance.defaults.headers = {
			...headersInstance,
		};
		try {
			const response = await axiosInstance.get(entityUrl);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},
	patch: async (entityUrl, jsonData) => {
		axiosInstance.defaults.headers = {
			...headersInstance,
		};
		try {
			const response = await axiosInstance.patch(entityUrl, jsonData);
			return successHandler(response);
		} catch (error) {
			return errorHandler(error);
		}
	},
	source: () => {
		// const CancelToken = await axiosInstance.CancelToken;

		const CancelToken = axios.CancelToken;
		const source = CancelToken.source();
		return source;
	},
};
export default request;
